...
def test_inventory_dump(mocker):
    ...
    # spwd.getsnam can't be used by non-root user normally.
    # Mock the implementation so that we ca test.
    mocker.patch('spwd.getspnam', return_value=mocker.Mock(sp_pwd='password'))

    # grp.getgall will return the values from the test machine.
